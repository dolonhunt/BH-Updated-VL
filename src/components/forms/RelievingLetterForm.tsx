...
const initialData: RelievingLetterFormData = {
  name: '',
  designation: '',
  department: '',
  ref_code: '',
...
function mapEmployeeToForm(emp: Employee): Partial<RelievingLetterFormData> {
  return {
    name: emp.name,
...
function onCalculate(data: RelievingLetterFormData): Partial<RelievingLetterFormData> {
  const duration = calculateDuration(data.joining_date, data.leaving_date)
  return { duration }
}

export function RelievingLetterForm() {
  const {
    formData, setField,
        ...
        <div className="p-4 space-y-4">
          ...
          {showFields && (
            <>
              ...
              {generated ? (
                <GeneratedBanner>✓ Relieving Letter generated. See live preview →</GeneratedBanner>
              ) : (
                <PrimaryActionButton onClick={handleGenerate}>Generate Relieving Letter</PrimaryActionButton>
              )}
            </>
          )}
        </div>
