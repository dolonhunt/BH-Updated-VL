...
export function BankIntroLetterForm() {
  ...
  return <div className="flex flex-col h-full"><div className="flex-1 overflow-y-auto"><div className="p-4 space-y-4"><EmployeeSelect employees={employees} value={selectedEmployeeId} onChange={handleEmployeeChange} />{selectedEmployeeId === '__new__' && <AddEmployeeFirstNotice />}{show && <><Separator /><div className="space-y-3"><SectionHeading>Employee Info</SectionHeading><div className="grid grid-cols-2 gap-x-3 gap-y-1"><FormField label="Name" value={formData.name} onChange={v => setField('name', v)} error={errors.name} /><FormField label="Designation" value={formData.designation} onChange={v => setField('designation', v)} error={errors.designation} /><FormField label="Department" value={formData.department} onChange={v => setField('department', v)} /><FormField label="Employee ID" value={formData.employee_id} onChange={v => setField('employee_id', v)} /><FormField label="Ref Code" value={formData.ref_code} onChange={v => setField('ref_code', v)} /><FormField label="Joining Date" type="date" value={formData.joining_date} onChange={v => setField('joining_date', v)} /></div></div><Separator /><div className="space-y-3"><SectionHeading>Bank Details</SectionHeading><div className="grid grid-cols-2 gap-x-3 gap-y-1"><FormField label="Bank Name" value={formData.bank_name} onChange={v => setField('bank_name', v)} error={errors.bank_name} /><FormField label="Branch" value={formData.bank_branch} onChange={v => setField('bank_branch', v)} /><FormField label="Letter Date" type="date" value={formData.date} onChange={v => setField('date', v)} /></div></div>{generated ? <GeneratedBanner>✓ Bank Intro Letter generated.</GeneratedBanner> : <PrimaryActionButton onClick={handleGenerate}>Generate Bank Intro Letter</PrimaryActionButton>}</>}</div></div><MismatchModal open={showMismatchModal} onClose={() => setShowMismatchModal(false)} mismatches={mismatches} onAction={handleMismatchAction} /></div>
}
